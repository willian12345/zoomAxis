...
function App() {
  ...
  const syncTrackWidth = () => {
    ...
    // 需要用 useRef 获取最新值
    trackScrollWidthRef.current =
      trackItemWidth < stageWidth ? stageWidth : trackItemWidth;
    // 会导致重新渲染
  ...
  const initApp = () => {
    ...
    trackCursor.addEventListener(CURSOR_POINTER_EVENT_TYPE.UPDATE, (e) => {
      // console.log(e);
  ...
  const add = (trackId: string) => {
    segmentTracks?.addSegmentWithFramestart(
      trackId,
      1,
  ...
  const TrackHeaderItem = ({ track }: { track?: TTrackConfig }) => {
      ...
      return (
        ...
        >
          ...
          {track.subTracks?.map((subTrack, index) => (
            <div className="track-operation-item" key={index}>
              {subTrack.trackText}
            </div>
          ))}
    ...
    return (
      <div className="track-operation-item" key={track?.trackId}>
        {track?.trackText}
      </div>
    );
  };
  ...
  return (
    <div className="App">
      <div className="wrapper">
        <div className="btns">
          <button onClick={splitHandler}>分割</button>
          <button onClick={toggleMagnet}>辅助线吸附</button>
        </div>
        <div className="segment-list" ref={segmentItemListRef}>
          <div
            className="segment-item"
            style={{ backgroundColor: "#C66136" }}
            data-segment-type={"1"}
          >
            拖我
            <button className="btn" onClick={() => handleAddByClick("1")}>
              +
            </button>
          </div>
          ...
          <div className="segment-item" data-segment-type={"3"}>
            拖我（一）
            <button className="btn" onClick={() => handleAddByClick("3")}>
              +
            </button>
          </div>
          <div className="segment-item" data-segment-type={"4"}>
            拖我
            <button className="btn" onClick={() => handleAddByClick("4")}>
              +
            </button>
          </div>
          {/* <div
        ...
        <div className="timeline-container" ref={timelineContainer}>
          ...
          >
            ...
            <div className="scroll-content" ref={scrollContentRef}>
              <div className="track-list" style={{ width: `${trackWidth}px` }}>
